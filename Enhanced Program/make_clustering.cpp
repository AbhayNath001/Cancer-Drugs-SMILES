...
int main(int argc, char *argv[]) {
    vector<string> prohibitive = read_restriction_folders("cannot_links.txt");
    if (prohibitive.size() == 2) {
        read_filenames_from_folder(prohibitive[0] + "_parsed_files.txt", 0);
        read_filenames_from_folder(prohibitive[1] + "_parsed_files.txt", 1);
    }

    vector<string> aggregating = read_restriction_folders("must_links.txt");

    if (aggregating.size() == 1) {
        read_filenames_from_folder(aggregating[0] + "_parsed_files.txt", 2);
    }

    vector<string> random = read_restriction_folders("no_links.txt");

    if (random.size() == 1) {
        read_filenames_from_folder(random[0] + "_parsed_files.txt", 3);
    }
    ...
    while (max_cluster < filenames.size()-1) {
        create_new_cluster();
        ofstream myfile;
        myfile.open(string(argv[1]) + "iter" + to_string(max_cluster) + ".txt");
    ...
    ofstream myfile;
    myfile.open(string(argv[1]) + "filenames.txt");
    for (int i=0;i<filenames.size();++i) {
